...
export default function CartDrawer() {
  ...
  const submit = async () => {
    if (!form.name.trim() || !form.phone.trim()) { setMsg('Вкажіть імʼя та телефон'); return }
    ...
    try {
      ...
      setMsg('Дякуємо! Замовлення прийнято.'); clear(); setCheckout(false)
    ...
    <>
      ...
      <aside style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 'min(420px,100%)', background: '#fff', boxShadow: '-10px 0 40px rgba(0,0,0,.1)', transform: isOpen ? 'translateX(0)' : 'translateX(100%)', transition: '.25s', zIndex: 61, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: 18, borderBottom: '1px solid var(--line)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <strong style={{ fontSize: 18 }}>Кошик</strong>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 18 }} className="hidden-scroll">
          {items.length === 0 ? <p className="muted">Кошик порожній</p> : items.map((it, i) => (
            <div key={i} style={{ display: 'flex', gap: 12, marginBottom: 14, paddingBottom: 14, borderBottom: '1px solid var(--line)' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <button onClick={() => remove(it.product.id, it.size, it.color)} className="muted" style={{ fontSize: 12, background: 'none', border: 'none', marginTop: 4 }}>Видалити</button>
              </div>
            </div>
          ))}
        </div>
        <div style={{ padding: 18, borderTop: '1px solid var(--line)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800, fontSize: 18, marginBottom: 12 }}><span>Разом:</span><span>{total} ₴</span></div>
          {!checkout ? (
            <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }} disabled={items.length === 0} onClick={() => setCheckout(true)}>Оформити замовлення</button>
          ) : (
            <div style={{ display: 'grid', gap: 8 }}>
              <input className="input" placeholder="Імʼя *" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
              ...
              <input className="input" placeholder="Місто / Відділення" value={form.delivery} onChange={e => setForm({ ...form, delivery: e.target.value })} />
              <select value={form.payment} onChange={e => setForm({ ...form, payment: e.target.value })}><option>Наложений платіж</option><option>Передоплата на картку</option></select>
              <textarea className="input" rows={2} placeholder="Коментар" value={form.comment} onChange={e => setForm({ ...form, comment: e.target.value })} />
              <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }} disabled={sending} onClick={submit}>{sending ? 'Надсилаємо...' : 'Підтвердити замовлення'}</button>
            </div>
          )}
